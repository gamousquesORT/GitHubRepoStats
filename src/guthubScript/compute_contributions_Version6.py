...
def files_touched_per_author(path, exts, skip_patterns=None):
    """
    ...
    for line in out.splitlines():
        ...
        if current_author and _matches_ext(line, exts):
            # apply skip patterns (ignore build artifacts and framework-generated files)
            if skip_patterns and any(p in line for p in skip_patterns):
                continue
...
def pretty_print_results(results, files_by_author, show_files, file_creator=None, file_top_contrib=None, file_creation_date=None, base_path=None):
    print()
    ...
    if show_files:
        ...
        print("\nLegend: '*' = file creator (author who added the file). '+' = top contributor (author with most current lines according to git blame). Both markers may appear together.")
        for r in results:
            author = r["author"]
            ...
            if fls:
                prev_dir = None
                for f in fls:
                    # compute a path relative to base_path for nicer grouping if provided
                    rel = f
                    if base_path:
                    ...
                    markers = ""
                    try:
                        if file_creator and file_creator.get(f) == author:
                            markers += "*"
                        if file_top_contrib and file_top_contrib.get(f) == author:
                            markers += "+"
                    except Exception:
                        markers = ""

                    # get creation date for the file
                    creation_date = ""
                    if file_creation_date and f in file_creation_date:
                        creation_date = file_creation_date[f]

                    # print the file name indented under its folder
                    display_name = os.path.basename(rel) if dirpath != '.' else rel
                    if creation_date:
                        if markers:
                            print("    {:<60}   {}   [Created: {}]".format(display_name, markers, creation_date))
                        else:
                            print("    {:<60}   [Created: {}]".format(display_name, creation_date))
                    else:
                        if markers:
                            print("    {:<60}   {}".format(display_name, markers))
                        else:
                            print("    {:<60}".format(display_name))
            else:
                print("(no files recorded)")


def compute_file_level_info(path, exts, skip_patterns=None):
    """
    For each tracked file under path (and matching exts), determine:
      - creator: author who added the file (git log --diff-filter=A -1)
      - creation_date: date when the file was added
      - top contributor: author with most current lines in git blame for that file

    Returns three dicts: (file_creator, file_top_contrib, file_creation_date)
    """
    if skip_patterns is None:
        skip_patterns = ['bin/', 'obj/']
    try:
    ...
    for f in files:
        ...
        # determine creator (author of the commit that added the file) and creation date
        try:
            creator = run(["git", "log", "--diff-filter=A", "-1", "--pretty=format:%an", "--", f]).strip()
            if creator:
                file_creator[f] = creator
            # get creation date and time (format: YYYY-MM-DD HH:MM:SS)
            creation_date = run(["git", "log", "--diff-filter=A", "-1", "--pretty=format:%ad", "--date=format:%Y-%m-%d %H:%M:%S", "--", f]).strip()
            if creation_date:
                file_creation_date[f] = creation_date
        except subprocess.CalledProcessError:
            # leave missing
            pass

        # determine top contributor by current lines owned via blame
        try:
            blame = run(["git", "blame", "-w", "-e", "--line-porcelain", f])
        except subprocess.CalledProcessError:
            continue
        ...
        for bl in blame.splitlines():
            if bl.startswith("author "):
                a = bl[len("author "):].strip()
                per_author[a] += 1
        if per_author:
            top = max(per_author.items(), key=lambda x: x[1])[0]
            file_top[f] = top

    return file_creator, file_top, file_creation_date
...
if __name__ == "__main__":
    ...
    # central skip patterns to ignore build and framework output
    skip_patterns = ['bin/', 'obj/', 'packages/', '.vs/', 'artifacts/']
    files_by_author = files_touched_per_author(path, exts, skip_patterns=skip_patterns)
    lines_owned = lines_owned_per_author(path, exts, skip_patterns=skip_patterns)
    ...
    # compute per-file creator (*) and top contributor (+) so we can mark files in the detailed output
    file_creator, file_top, file_creation_date = compute_file_level_info(path, exts, skip_patterns=skip_patterns)
    pretty_print_results(results, files_by_author, args.output_files_per_author, file_creator, file_top, file_creation_date, base_path=path)